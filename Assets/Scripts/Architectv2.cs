...
public class Architectv2 : MonoBehaviour {
...
//			}
	...
	void Destructor()
	{
		...
		longPool.DestroyAll();
		tallPool.DestroyAll();
		Controllerv2.FireEvent(maxHeight);
		blocks.Clear();
	...
	void Interpretor(Rules rule)
	{
		...
		case Rules.FORWARD :
			...
			if(!Collide(transform.forward,-transform.up))
				positionToReach = positionToReach - transform.up;
			break;
		case Rules.BACKWARD :
			if(!Collide( -transform.forward))
			{
		...
		case Rules.LEFT :
			...
			if(!Collide(-transform.right,-transform.up))
				positionToReach = positionToReach - transform.up;
			break;
		case Rules.RIGHT :
			if(!Collide( transform.right))
			...
			break;
		case Rules.UP :
			if(!Collide(transform.up,transform.forward) && Collide(transform.forward))
			{
				positionToReach = transform.position + transform.up + transform.forward;
			}
			break;
//		case Rules.DOWN :
...
//			break;
		...
		case Rules.DROPCUBE :
			if(!Collide(transform.forward))
			{
				GameObject obj = cubePool.GetPooledObject();
		...
		case Rules.DROPLONGRIGHT :
			...
			{
				GameObject obj = longPool.GetPooledObject();
		...
		case Rules.DROPTALLUP :
			...
			{
				GameObject obj = tallPool.GetPooledObject();
...
//		int nbGenes = Random.Range(1,maximumGene);
	...
	public void CrossOver(Architectv2 other)
	{
